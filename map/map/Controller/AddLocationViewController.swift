...
//
    ...
    func handlePublicUserData(firstName : String?, lastName : String? , error : Error? ){
        if error == nil {
            UdacityClient.postStudentLocation(firstName: firstName ?? "", lastName: lastName ?? "" , mapString: location, mediaURL: link, latitude: Float(latitude), longitude: Float(longitude), completion: handlePostStudentLocation(success:error:))
        } else {
